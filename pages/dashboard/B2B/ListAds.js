...
import { b2bFilter } from "~static/FormData/b2bForms/";
...
const B2bSearch = (props) => {
	...
	function applyFilter(payload, page) {
		...
		if (payload["b2b_member_id"]) {
			obj["b2b_member_id"] = payload["b2b_member_id"];
		} else {
									...
									if (values.b2b_member_id) {
										...
										if (values["booking_date"]) {
											...
											Object.keys(values).forEach((k) => {
												if (k != "b2b_member_id" && k != "booking_date") {
													if (!values[k]) {
