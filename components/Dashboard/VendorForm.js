	...
	const _handleSubmit = ({
		...
		if (values.catalog_pdf_path) {
			...
			for (var i = 0; i < valKeyArr.length; i++) {
				if (vendorData[valKeyArr[i]] != values[valKeyArr[i]]) {
					proceed = true;
					break;
				}
			}
		...
		if (id) {
			payload.append("vendor_id", id);
		}
		if (proceed) {
			if (payload) {
				vendorActions
					.createVendor(payload, id)
					.then(function (response) {
						setSubmitting(false);
						console.log("ressss", response);
						if (response.data.data.id) {
							...
							setProfileUpdateSuccess({
								...
								submitText: "Done",
								submitURL:
									routerLink.starter.vendornew + "?id=" + response.data.data.id,
							});
							setCatalogFile(response.data.data.catalog_pdf_path);
							resetForm();
						}
			...
			<Grid container spacing={1}>
				...
				<Grid item xs={12}>
					<Link
						...
						href={routerLink.starter.vendorImg + "?id=" + id}
					>
						<Accordion
							expanded={false}
				...
				<Grid item xs={12}>
					<Link
						style={{ textDecoration: "none", color: "black" }}
						href={routerLink.starter.vendorVids + "?id=" + id}
					>
						<Accordion
							expanded={false}
							disabled={props.router.query.id == null}
